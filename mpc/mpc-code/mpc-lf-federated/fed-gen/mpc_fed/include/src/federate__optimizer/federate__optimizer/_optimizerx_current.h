...
#ifndef __OPTIMIZERX_CURRENT_H // necessary for arduino-cli, which automatically includes headers that are not used
...
#endif
typedef struct _optimizerx_current_self_t{
    self_base_t base; // This field is only to be used by the runtime, not the user.
    int end[0]; // placeholder; MSVC does not compile empty structs
} _optimizerx_current_self_t;
typedef struct {
    token_type_t type;
    lf_token_t* token;
    size_t length;
